...
{
  Nodes <- x$Nodes
  links.no <- which(unlist(lapply(Nodes, function(x) class(x)[1])) == "cat_Link")
  clims.no <- which(unlist(lapply(Nodes, function(x) class(x)[1])) == "cat_Climate")
  outlet.no <- which(unlist(lapply(Nodes, function(x) class(x)[1])) == "cat_Outlet")
  nodes <- Nodes[-c(links.no, clims.no)]
  links <- Nodes[links.no]
